...
exports.BasePage = class BasePage {
  ...
  textLoc(name) {return `//*[normalize-space(text())='${name}']`}
  ...
  async linkGiper(nameFrame, locator, expectedLink) {
    console.log(`Проверяем привзку href '${expectedLink}' к наименовнию '${locator}'`);
    const frame = await this.page.frame({ name: nameFrame });
    await frame.waitForLoadState(); // ожидаем загрузки фрейма   
    const links = await frame.locator(`//*[normalize-space(text())='${locator}']`);
