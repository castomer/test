...
exports.DonatForm = class DonatForm extends BasePage {
  ...
  async linkCheck() {
    await this.linkGiper(this.frameName, "Other Ways to Give", "https://demo.fundraiseup.com") // Проверка линка формы A
    await this.linkGiper(this.frameName, "How We Use Your Gift", "https://demo.fundraiseup.com") // Проверка линка формы A
    await this.linkGiper(this.frameName, "Manage Your Donations", "https://donor.helping-hand.org/demo") // Проверка линка формы A
  }
  ...
  async  selectOptionInFrame(nameFrame, selector) {
    ...
    await frame.selectOption('select.currency-select-control', { value: 'USD' }); // Выбор опции по значению   
    await frame.waitForSelector('//span[@data-qa="selected-currency" and text()="USD"]', { state: 'visible' }); // Ожидаем видимость элемента
  }
